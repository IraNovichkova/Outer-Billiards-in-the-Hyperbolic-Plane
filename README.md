...
В файле README.md я расскажу о том, что такое внешний бильярд и в чём заключается главный вопрос моей исследовательской работы. В файле course_work.py код с комментариями, который можно запустить у себя и получить визуализацию бильярда (а также исследовать бильярд, подставляя разные значения параметров).
...
Меняя значения параметров, а именно размеры треугольника и количество итераций, я доказала теорему:
- Для отображения правого внешнего бильярда относительно правильного треугольника с центром в нуле и вершинами $z_1 = \epsilon, z_2 = \epsilon e^{\frac{2\pi i}{3}}, z_3 = \epsilon e^{\frac{-2\pi i}{3}}$, таких что $\epsilon < 2 - \sqrt{3}$, существует цикл длины 3. Точки, образующие цикл, имеют вид
   z_a = e^{-\frac{\pi i}{3}} a,
   ...
   a = \frac{1 + \epsilon^2 - \sqrt{\epsilon^4 - 14\epsilon^2 + 1}}{4\epsilon}
   
   если $\epsilon \geqslant 2 - \sqrt{3}$, то периодических точек с периодом 3 в $\mathbb{D}$ нет.
